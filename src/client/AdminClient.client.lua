...
local giveFishEvent = remoteFolder:WaitForChild("GiveFish", 5)
...
local totalTabs = hasPrimaryAccess and 4 or 3  -- 4 tabs for primary admin (includes Log), 3 for secondary

local function createTab(name, order)
	local tab = Instance.new("TextButton")

	-- Scale-based width with gap (4 tabs for primary admin, 3 for secondary)
	local tabWidth = hasPrimaryAccess and 0.24 or 0.32
...
-- ✅ GIVE FISH POPUP - Allows admin to give fish to players
local function showGiveFishPopup(targetPlayer)
	-- Load FishConfig
	...
	local popup = Instance.new("Frame")
	popup.Size = UDim2.new(0, 420, 0, 500)
	popup.Position = UDim2.new(0.5, 0, 0.5, 0)
	popup.AnchorPoint = Vector2.new(0.5, 0.5)
	popup.BackgroundColor3 = COLORS.Background
	popup.BorderSizePixel = 0
	...
	title.BackgroundTransparency = 1
	title.Font = Enum.Font.GothamBold
	title.Text = "🐟 Give Fish to " .. targetPlayer.Name
	title.TextColor3 = COLORS.Text
	title.TextSize = 15
	title.TextXAlignment = Enum.TextXAlignment.Left
	title.Parent = header
	...
	-- Fish Selection ScrollingFrame
	local fishScroll = Instance.new("ScrollingFrame")
	fishScroll.Size = UDim2.new(1, 0, 1, -60)
	fishScroll.Position = UDim2.new(0, 0, 0, 0)
	fishScroll.BackgroundColor3 = COLORS.Panel
	fishScroll.BorderSizePixel = 0
	fishScroll.ScrollBarThickness = 4
	fishScroll.ScrollBarImageColor3 = COLORS.Border
	fishScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
	fishScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
	fishScroll.Parent = contentContainer

	createCorner(8).Parent = fishScroll
	createPadding(8).Parent = fishScroll

	local fishLayout = Instance.new("UIListLayout")
	fishLayout.Padding = UDim.new(0, 5)
	fishLayout.SortOrder = Enum.SortOrder.Name
	fishLayout.Parent = fishScroll

	-- Selected fish tracking
	local selectedFish = {}

	-- Create fish items from FishConfig
	if FishConfig and FishConfig.Fish then
		for fishId, fishData in pairs(FishConfig.Fish) do
			local fishFrame = Instance.new("Frame")
			fishFrame.Name = fishId
			fishFrame.Size = UDim2.new(1, 0, 0, 35)
			fishFrame.BackgroundColor3 = COLORS.Button
			fishFrame.BorderSizePixel = 0
			fishFrame.Parent = fishScroll

			createCorner(6).Parent = fishFrame
			...
			checkbox.BackgroundColor3 = COLORS.Panel
			checkbox.BorderSizePixel = 0
			checkbox.Text = ""
			checkbox.Parent = fishFrame

			createCorner(4).Parent = checkbox
			...
			nameLabel.Text = fishData.Name or fishId
			...
			qtyBox.Size = UDim2.new(0, 50, 0, 25)
			qtyBox.Position = UDim2.new(1, -60, 0.5, 0)
			qtyBox.AnchorPoint = Vector2.new(0, 0.5)
			qtyBox.BackgroundColor3 = COLORS.Panel
			qtyBox.BorderSizePixel = 0
			qtyBox.Font = Enum.Font.Gotham
			qtyBox.PlaceholderText = "1"
			qtyBox.Text = "1"
			qtyBox.TextColor3 = COLORS.Text
			qtyBox.TextSize = 12
			qtyBox.Parent = fishFrame

			createCorner(4).Parent = qtyBox

			checkbox.MouseButton1Click:Connect(function()
				if selectedFish[fishId] then
					selectedFish[fishId] = nil
					checkmark.Visible = false
					checkbox.BackgroundColor3 = COLORS.Panel
				else
					local qty = tonumber(qtyBox.Text) or 1
					selectedFish[fishId] = {id = fishId, quantity = qty}
					checkmark.Visible = true
			...
			qtyBox.FocusLost:Connect(function()
				if selectedFish[fishId] then
					local qty = tonumber(qtyBox.Text) or 1
					selectedFish[fishId].quantity = qty
				end
			end)
		end
	end
	...
	-- Give Fish Button
	local giveBtn = createButton("🐟 Give Selected Fish", COLORS.Success, Color3.fromRGB(77, 191, 139))
	giveBtn.Size = UDim2.new(1, 0, 0, 45)
	giveBtn.Position = UDim2.new(0, 0, 1, -50)
	giveBtn.Parent = contentContainer

	giveBtn.MouseButton1Click:Connect(function()
		-- Build fish list
		local fishList = {}
		for fishId, data in pairs(selectedFish) do
			table.insert(fishList, data)
		end

		if #fishList == 0 then
			StarterGui:SetCore("SendNotification", {
				Title = "❌ No Fish Selected",
				Text = "Please select at least one fish to give",
				Duration = 3
			})
			return
		end
		...
		showConfirmation(
			"Give Fish",
			string.format("Give %d fish type(s) to %s?", #fishList, targetPlayer.Name),
			function()
				if giveFishEvent then
					giveFishEvent:FireServer(targetPlayer.UserId, fishList)
					print(string.format("[ADMIN CLIENT] Gave fish to %s", targetPlayer.Name))
				end
				popup:Destroy()
				mainContainer.Visible = true
			end
...
local function createPlayerCard(targetPlayer)
	...
	card.MouseButton1Click:Connect(function()
		...
		for _, child in ipairs(detailScroll:GetChildren()) do
			...
			-- ✅ Give Fish Button (untuk fishing game)
			local giveFishBtn = createButton("🐟 Give Fish", COLORS.Accent, Color3.fromRGB(128, 141, 255))
			giveFishBtn.LayoutOrder = 8
			giveFishBtn.Parent = actionsFrame
			giveFishBtn.MouseButton1Click:Connect(function()
				if targetPlayer then
					playerDetailPanel.Visible = false
					showGiveFishPopup(targetPlayer)
				end
			end)
...
-- Leaderboard Tab (was Events at position 3, now Leaderboard at 3)
local leaderboardTab, leaderboardTabBtn = createTab("Leaderboard", 3)
...
local currentLeaderboardType = "fish"
local leaderboardSubButtons = {}

local function createSubCategoryButton(text, leaderboardType, icon)
	local btn = Instance.new("TextButton")
	btn.Size = UDim2.new(0.48, -5, 1, 0) -- Bigger buttons for fewer categories
...
local fishCaughtBtn = createSubCategoryButton("Fish Caught", "fish", "🐟")
...
local function createLeaderboardCard(data)
	...
	local statsY = 75
	local stats = {
		{icon = "🐟", label = "Fish Caught", value = tostring(data.FishCaught or 0)},
		{icon = "💎", label = "Donate", value = "R$" .. tostring(data.Donation or 0)}
	}
	...
	deleteBtn.MouseButton1Click:Connect(function()
		...
		local yPos = 65
		local deleteOptions = {
			{text = "Delete Fish Caught Data", type = "fish"},
...
if hasPrimaryAccess then
	logTab, logTabBtn = createTab("Log", 4)
	...
	logDesc.Text = "Monitor admin actions across all servers. Track kicks, bans, freezes, title changes, fishing data modifications, and notifications."
