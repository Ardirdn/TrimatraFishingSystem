...
local function rebuildAreaCache()
	...
	if not fishAreaModel then
		warn("[FISH AREA SYSTEM] 'Location' model not found in workspace!")
		return
	end
...
function FishAreaSystem.GetRandomFishInArea(position)
	...
	if position then
		...
		if areaName then
			...
			print("[FISH AREA] Fishing in Open Waters (no special area)")
		end
	end
	...
	if #fishPool == 0 then
		if areaName == nil then
			...
			print(string.format("[FISH AREA] No %s fish in Open Waters, falling back to Anywhere fish", selectedRarity))
			...
			print(string.format("[FISH AREA] No %s fish in %s area, trying Anywhere fish", selectedRarity, areaName))
			...
			if #fishPool == 0 then
				print(string.format("[FISH AREA] Still no fish, falling back to any Anywhere fish"))
				for fishId, fishData in pairs(FishConfig.Fish) do
					if fishData.Location == "Anywhere" then
						table.insert(fishPool, { fishId = fishId, weight = 1 })
						poolWeight = poolWeight + 1
