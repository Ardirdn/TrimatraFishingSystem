...
function fetchShopData()
	if not getShopDataFunc then 
		warn("[ROD SHOP CLIENT] getShopDataFunc is nil!")
		return 
	end
	...
	if success and data then
		shopData = data
		-- ✅ DEBUG: Log shop data for troubleshooting
		print(string.format("🎣 [ROD SHOP CLIENT] Fetched shop data - Rods: %d, Floaters: %d, Money: %d", 
			#RodShopConfig.Rods, #RodShopConfig.Floaters, data.Money or 0))
		updateShopDisplay()
	else
		warn("[ROD SHOP CLIENT] Failed to fetch shop data:", tostring(data))
	end
end
