...
getShopDataFunc.OnServerInvoke = function(player)
	...
	-- ✅ FIX: Use config-based default rod ID instead of hardcoded
	local defaultRodId = RodShopConfig.GetDefaultRod and RodShopConfig.GetDefaultRod() or "WoodRod"
	
	return {
		OwnedRods = data.OwnedRods or {defaultRodId},
		OwnedFloaters = data.OwnedFloaters or {},
		EquippedRod = data.EquippedRod or defaultRodId,
...
buyRodEvent.OnServerEvent:Connect(function(player, rodId)
	...
	if not data.OwnedRods then
		DataHandler:Set(player, "OwnedRods", {defaultRodId})
