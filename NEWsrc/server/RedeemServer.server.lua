...
local TitleServer = require(script.Parent.TitleServer) -- ✅ ADDED: For applying privileges
...
redeemCodeEvent.OnServerEvent:Connect(function(player, codeString)
	...
	if rewardType == "Title" then
		...
		-- Give title
		-- ✅ FIX: Use UnlockedTitles (not OwnedTitles) for consistency with TitleServer
		...
		-- Also add to OwnedTitles for backward compatibility
		...
		-- ✅ FIX: APPLY PRIVILEGES (give tools like SpeedCoil for VIP, etc.)
		task.spawn(function()
			task.wait(0.5) -- Wait for data to save
			TitleServer:ApplyPrivileges(player, rewardValue)
			
			-- ✅ Also update team and broadcast
			if TitleServer.UpdatePlayerTeam then
				TitleServer:UpdatePlayerTeam(player, rewardValue)
			end
			if TitleServer.BroadcastTitle then
